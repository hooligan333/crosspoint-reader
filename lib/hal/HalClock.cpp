...
bool HalClock::localTime(struct tm& out) const {
  ...
  // System time is UTC; the display zone is the process-wide TZ rule that
  // setTimezone() installs, exactly as on the RTC backend.
...
bool HalClock::syncFromNTP() {
  ...
  // remember the display timezone so every exit below can restore it.
  ...
  for (int i = 0; i < maxAttempts; i++) {
    if (sntp_get_sync_status() == SNTP_SYNC_STATUS_COMPLETED) {
      ...
      Rtc::DateTime dt;
      if (!getDateTime(dt)) {  // unreachable: the reading above is plausible
        setTimezone(savedTz);
        return false;
      }
      g_rail.successMs = railNowMs();
...
// The timezone rule and everything that presents local time sit on top of
// localTime(), so they are written once.
