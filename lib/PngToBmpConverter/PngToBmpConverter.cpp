...
bool PngToBmpConverter::pngFileToBmpStreamInternal(HalFile& pngFile, Print& bmpOut, int targetWidth, int targetHeight,
  ...
  if (oneBit) {
    atkinson1BitDitherer = makeUniqueNoThrow<Atkinson1BitDitherer>(emitWidth);
    ...
    if (USE_ATKINSON) {
      atkinsonDitherer = makeUniqueNoThrow<AtkinsonDitherer>(emitWidth, originalThresholds);
      ...
      fsDitherer = makeUniqueNoThrow<FloydSteinbergDitherer>(emitWidth, originalThresholds);
  ...
  if (needsScaling) {
    // outWidth is source-derived and unbounded in crop mode (an extreme-aspect source can
    // scale to hundreds of thousands of columns); a throwing new[] here would abort
    // instead of failing the conversion like the JPEG converter does on OOM
    rowAccum = new (std::nothrow) uint32_t[outWidth]();
    rowCount = new (std::nothrow) uint16_t[outWidth]();
    if (!rowAccum || !rowCount) {
      LOG_ERR("PNG", "Failed to allocate scaling buffers (%d px)", outWidth);
      delete[] rowAccum;
      delete[] rowCount;
      // The ditherers are unique_ptr now (#3521) and free themselves on return.
      free(rowBuffer);
      free(ctx.currentRow);
      free(ctx.previousRow);
      return false;
...
bool PngToBmpConverter::pngFileToBmpStreamWithSize(HalFile& pngFile, Print& bmpOut, int targetMaxWidth,
                                                   int targetMaxHeight) {
  return pngFileToBmpStreamInternal(pngFile, bmpOut, targetMaxWidth, targetMaxHeight, false, /*crop=*/true,
                                    /*originalThresholds=*/false, /*boundOutputToTarget=*/true);
}

bool PngToBmpConverter::pngFileTo1BitBmpStreamWithSize(HalFile& pngFile, Print& bmpOut, int targetMaxWidth,
                                                       int targetMaxHeight) {
  return pngFileToBmpStreamInternal(pngFile, bmpOut, targetMaxWidth, targetMaxHeight, true, /*crop=*/true,
                                    /*originalThresholds=*/false, /*boundOutputToTarget=*/true);
}
