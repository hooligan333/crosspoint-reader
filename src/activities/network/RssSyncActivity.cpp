...
bool RssSyncActivity::preventAutoSleep() {
  switch (state) {
...
void RssSyncActivity::runSync() {
  ...
  for (auto& row : rows) {
    ...
    if (!ok) {
      // A transfer the user aborted is not a failure; end the run and let the
      // summary report the remainder as cancelled.
      if (cancelRequested) {
        cancelled = true;
        break;
      }
...
bool RssSyncActivity::downloadRow(const Row& row) {
  ...
  const auto result = HttpDownloader::downloadToFile(
      ...
      // multi-MB enclosure over a slow LAN the between-items check leaves the
      // screen unresponsive for minutes. Safe because the transfer lands in
      // tmpPath and is only renamed into place once complete, so an aborted
      // item never leaves a half file that reads as a book.
      &cancelRequested);

  if (result == HttpDownloader::ABORTED) {
    LOG_DBG("RSS", "Download cancelled: %s", row.url.c_str());
...
void RssSyncActivity::buildListScreen(UiScreen& screen) {
  ...
  props.valueInset = 8;
  // No third argument: #3366 repurposed it from `bool hasSubtitle` to
  // `int selectionOffset` (leading ring entries such as a tab bar). The
  // subtitle row height it used to request is now derived in the SDK from
  // measured content over tokens.listMinRowHeight, so there is nothing to
  // replace it with -- and a stray `true` would draw the highlight one row
  // above the row Confirm activates.
