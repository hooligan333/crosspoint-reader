...
// this firmware does not have to spare, and the log wants raw RTC time anyway.
uint32_t daysFromCivil(uint32_t year, const uint32_t month, const uint32_t day) {
  year -= month <= 2;
...
void civilFromDays(uint32_t days, uint16_t& year, uint8_t& month, uint8_t& day) {
  days += 719468;
...
uint32_t readWallClock() {
  ...
  return daysFromCivil(dt.year, dt.month, dt.day) * SECONDS_PER_DAY + dt.hour * 3600u + dt.minute * 60u + dt.second;
}
...
bool UsageLog::writeRow(HalFile& file, const Entry& e, const uint8_t event, const uint8_t aux, const uint32_t flushWall,
  ...
  if (flushWall != 0) {
    ...
    uint8_t month;
    uint8_t day;
    civilFromDays(wall / SECONDS_PER_DAY, year, month, day);
