...
// The stock idle poll (#3463): sleep in short slices and wake the poll as soon
// as a button contact closes. InputManager commits a press only when two
// consecutive polls agree, so a press shorter than one 50 ms sleep could land in
// a single sample and be lost. Extracted so the CROSSPOINT_TOUCH_INT_WAKE
// fallback below keeps exactly this behaviour rather than a plain delay(50).
static void idleInputSlicePoll() {
